...
function createStartScreen() {
    ...
    const title = this.add.text(centerX, centerY - 100, 'ZORO', {
        fontFamily: '"Comic Neue", cursive',
        fontSize: '120px', // Increased from 80px to 120px
        fontStyle: 'bold',
        color: '#ffffff',
        stroke: '#000000',
        strokeThickness: 6,
...
function createMemorizePhase() {
    ...
    const cardWidth = Math.min(width * 0.8 / gridSize, 150);
    ...
    for (let i = 0; i < cardsToMemorize; i++) {
        ...
        const shadow = this.add.rectangle(x + 5, y + 5, cardWidth, cardHeight, 0x000000, 0.3);
...
function gameOver(success) {
    ...
    // Game over message
    const message = success ? 'Congratulations!' : 'Game Over!';
